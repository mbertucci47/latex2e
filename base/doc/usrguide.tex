...
firstly a programming language for \LaTeX{} (L3 programming layer, L3PL) and then a range of
...
kernel. The fact that `behind the scenes' they are built on the L3PL
...
\begin{itemize}
  \item[\texttt{m}] A standard mandatory argument, which can either be a single
    token\footnote{A single character or command name}
...
description also gives special meaning to four other characters.
...
be interpreted as a series of key--values (keyvals). See Section~\ref{sec:cmd:keyval}
for more details.
...
\subsection{Auto-converting to key--value (keyval) format}
...
etc. Introducing more sophisticated (key--value, `keyval') options to these commands
...
as the key in a keyval pair \emph{if} the `raw' argument does \emph{not}
...
initial delimiter by peeking ahead (using the L3PL's |\peek_...|
...
\begin{verbatim}
   ...
   \ExplSyntaxOff
\end{verbatim}
[As an aside: the code is written in the L3PL, so we don't have to
...
adjustment of the information printed using a keyval approach
\begin{description}
